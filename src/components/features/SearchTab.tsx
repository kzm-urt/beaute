    ...
    <>
      ...
      <div className="px-4 pt-3">
        ...
        <div className="flex gap-1.5 overflow-x-auto hide-scrollbar pb-2">
          {["すべて", ...Object.keys(CAT_META)].map((c) => (
            <Chip key={c} label={c === "すべて" ? c : `${CAT_META[c as Category]?.icon} ${c}`}
              active={activeCat === c} onClick={() => setActiveCat(c)} />
          ))}
        </div>
        ...
        <button
          ...
          style={{ color: "#A8722A" }}>
          {showTags ? "▾" : "▸"} タグで絞り込む
          {activeTags.length > 0 && ` (${activeTags.length}件)`}
        </button>
        {showTags && (
